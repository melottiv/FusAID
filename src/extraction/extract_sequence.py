...
def parse_gtf_basic_protein_coding(gtf_path):
    ...
    # 1) collect all transcript basic ∧ protein_coding
    basic_pc_txs = set()
    # save gene_name and synonyms on transcript lines
    tx_to_gene = {}       # tx_id -> gene_name (if present)
    ...
    with open(gtf_path, 'r') as fh:
        for line in fh:
            ...
            # basic and protein_coding
            basic_pc_txs.add(tx)

            # save gene_name/syn if present on transcript line 
    ...
    # 2) build gene_coords ONLY for these transcript,
    # but use gene_name also from 'transcript' lines, when present
    gene_coords = {}
    ...
    with open(gtf_path, 'r') as fh:
        for line in fh:
            ...
            if tx not in basic_pc_txs:
                continue  # only filtered transcripts
            ...
            # find gene_name: first look in tx_to_gene previously saved,
            # otherwise take gene_name from the current line if present.
            ...
            # synonyms: first from tx_to_synonyms, otherwise from the current line
            ...
            # mapping gene -> transcript (using gene_name and synonyms)
            if gene_name:
                gene_to_transcripts[gene_name].append(tx)
            for s in synonyms:
                gene_to_transcripts[s].append(tx)

    # sort CDS according to strand
